...
class AdminCommentManager extends AbstractManager
{
    ...
    {
        return $this->pdo->query("SELECT comment.id, comment.content, article.title, user.lastname  FROM comment INNER JOIN article ON article.id = comment.article_id INNER JOIN user ON user.id=comment.user_id;", \PDO::FETCH_CLASS, $this->className)->fetchAll();
    }
